...
endpoints: [
  {
    ...
    handler: async (req: any) => {
      ...
      if (!code) return Response.json({ message: 'Missing invite code' }, { status: 400 })
      ...
      if (!targetFamily) return Response.json({ message: 'Invalid invite code' }, { status: 404 })
      ...
      const currentMembers: string[] = Array.isArray((targetFamily as any).members)
        ? (targetFamily as any).members.map((m: any) => (typeof m === 'object' ? m?.id : m)).filter(Boolean)
        : []
      ...
      if (oldFamilyId && String(oldFamilyId) !== String(targetId)) {
        ...
        if (oldFamily) {
          const oldMembers: string[] = Array.isArray((oldFamily as any).members)
            ? (oldFamily as any).members.map((m: any) => (typeof m === 'object' ? m?.id : m)).filter(Boolean)
            : []
    ...
    beforeValidate: [
      ({ data, req }) => {
        ...
        if (
          req.user &&
          isCustomer(req) &&
          (!nextData.members || (Array.isArray(nextData.members) && nextData.members.length === 0))
        ) {
